...
**For whoever opens the editor and wants to build a skill tree without necessarily knowing the code.** And opening it is one click: [**the live editor**](https://cancioneschorriscortas-max.github.io/yggdrasil-forge/app/) runs entirely in your browser, no account, nothing to install. Editor state: 1.x — the Studio shipped with 1.0.

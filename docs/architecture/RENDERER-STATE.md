...
| **Renderer · sub-fase 1 — Fill por estado** | **O corpo do nodo tamén fala**, non só o anel. Tres adicións: (1) **5 tokens novos opcionais en `ThemeColors`** (`nodeFillLocked/Unlockable/Unlocked/Maxed/InProgress`), (2) **`visualStateFor(state, tier, maxTier)`**: helper puro que deriva `'in_progress'` para multi-tier a medias — cosmético, non toca o motor — e (3) **`fillColorForState(theme, visualState, nodeColor?)`** con prioridade `node.color > nodeFill<State> > nodeFill > '#f4f4ef'`. **`NodeDef.color`** xa tinguir o corpo (antes ignorábase). Cero regresión: sen tokens + sen `node.color` → idéntico ao previo. Theme Lab estendido con 5 pickers de fill por estado; preset escuro showcasea fills diferenciados (locked apagado, in_progress dourado tenue, maxed dourado pleno). 19 tests novos. | ✅ pushed |
| **Nivel · Capa A — `grantResource`** | **`TreeEngine.grantResource(resourceId, amount)`**: mutación directa de recurso, clampeada a `[0, max]`. Enabler do sistema de nivel: con `resource_min` (xa existe) gata nodos por valor; o Inspector explica automaticamente «Necesitas 10 en level, tes 3». Distinto de cost/refund: cero acoplamento con unlocks. Novo `ErrorCode.UNKNOWN_RESOURCE`. 8 tests novos; lección MASTER A.6.29. | ✅ pushed |
| **Rexións + Theme Lab á dereita (Capa 2)** | **Cada columna lóxica, o seu tinte.** Tres pezas: (1) **`SkillRegions.tsx`** novo subcompoñente que pinta `<rect>` con tinte por bbox de cada tag — z-order: rexións → edges → nodos, dentro do `<g transform>` (móvense co viewport), `pointerEvents="none"`. (2) **`SkillTree.regions?: RegionSpec[]`** prop opcional. Sen rexións → cero render extra. (3) **Theme Lab pasa a panel á dereita do sidebar**, sen despregable cara abaixo; novo **selector de columna** (Guerreiro/Paladín/Clérigo) que edita a cor da rexión activa en vivo; **cor de bloqueo segue dentro deste mesmo panel**. Demo: tres tags (warrior/paladin/cleric) repartidos polos 13 nodos do paladín. Layout CSS grid 3 columnas con breakpoints. 9 tests novos. | ✅ pushed |
| **Nivel · Capa B — recurso `level` + control +/- + gates** | **O nivel que abre portas.** Recurso `level` 1→10 engadido ao `tree-def-paladin` (sen `cost`, só axustable). Control **+/-** no panel Status que invoca `engine.grantResource('level', ±1)` (clamp [0,10] no motor). Dous gates con `resource_min`: **Pacto Escuro maldito** require nivel 10 (combinado en `all` co prereq orixinal); **Veterano de Guerra** require nivel 3 (combinado co `nodes_count(4, warrior)`). O Inspector explica automaticamente «Necesítanse N de level, tes X» grazas a explainUnlock (sen código extra). Coherente con A.6.28: **baixar nivel non re-bloquea** nodos xa abertos (prereqs = portas en unlock, non invariantes). Cero `@core`/`@react`; só demo. Verificado externamente con 7 casos. | ✅ pushed |
| **BUGFIX A.6.30 — exclusións bidireccionais** | **A incompatibilidade vai nos dous sentidos.** `canUnlock` e `unlock` antes só percorrían `nodeDef.exclusions`; iso permitía que se desbloqueasen X e Y mutuamente excluíntes se Y declaraba `exclusions: []`. Fix: índice inverso `Map<string, Set<string>>` construído unha vez no constructor; helper privado `findActiveExclusionConflict(nodeId, state)` que percorre direct ∪ inverse; **canUnlock e unlock usan o mesmo helper** (regression-proof). API pública `getEffectiveExclusions(nodeId): readonly string[]` (deduplicada, ordenada) para que a UI mostre incompatibilidades reais en ambas direccións. Cero cambio de schema. 11 tests novos (incluído test crítico do Camiño B). | ✅ pushed |
| **UI polish — Inspector exclusións + paleta + xa-unlocked + label de recursos** | Catro arranxos no demo, **cero motor/react**. (1) **Exclusións** usan `getEffectiveExclusions` (BUGFIX A.6.30) e xa **non** marcan con ✗ — distínguese ⛔ activo (vermello) vs ⚔️ declarado neutro; lese coma relación, non coma erro. (2) **Paleta presetDarkClean** distinguible por estado: gris/cián/ámbar/verde/morado; fills suaves; `nodeFillLocked` apagado para que os bloqueados se vexan "off" de corpo enteiro. (3) **Veredicto cando o nodo xa está unlocked**: caixa neutra «✓ Xa desbloqueado» (con "(maxed)" se aplica) en vez da vermella de erro; ese non é un erro, é estado normal. (4) **Helper `localizeResourceIds`** substitúe `"<resourceId>"` cru polo `"<label>"` localizado nas mensaxes do motor — agora le «Tienes al menos 10 de "Nivel"» en vez de «"level"». Cambio só no demo (`ConditionInspector.tsx` + `ThemeLab.tsx`); o motor pode seguir xerando ids crus porque é independente do locale. | ⏳ neste patch |

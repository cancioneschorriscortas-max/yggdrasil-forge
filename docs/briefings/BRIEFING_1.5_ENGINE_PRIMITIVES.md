# BRIEFING EXECUTABLE — Sub-fase 1.5
## Yggdrasil Forge: Primeira lóxica do motor — EventEmitter, ConcurrencyGuard, helpers
...
- `@yggdrasil-forge/core` con **todos os tipos fundamentais** (ondas 1, 2, 3 — sub-fases 1.2, 1.3, 1.4)
- CI verde, 39 tests pasan
- Documento mestre en `docs/architecture/MASTER.md`

**Esta é a primeira sub-fase con lóxica real do motor.** Ata agora foron tipos (datos puros). Agora hai clases con comportamento.

---

## 2. QUEN ES TI

Es un chat executor encargado **só desta sub-fase 1.5**. Non improvisas, non preguntas. Ao final, reportas no formato da sección 9.

⚠️ **IMPORTANTE:** Antes de empezar, lee no documento mestre (`docs/architecture/MASTER.md`):
- **Sección 10** (TreeEngine — para entender como se usa o EventEmitter)
- **Sección 46** (Concurrency e thread safety)
- **Sección 1.3** e **1.3.2** (convencións e reglas de Biome)
- A estrutura interna de @core en **sección 6.1**

---

## 3. OBXECTIVO DESTA SUB-FASE

Crear as tres pezas fundacionais do motor en `packages/core/src/engine/` e `packages/core/src/utils/`:

1. **`EventEmitter`** (`engine/EventEmitter.ts`) — sistema de eventos tipado sobre o `EventMap`
2. **`ConcurrencyGuard`** (`engine/ConcurrencyGuard.ts`) — serializa operacións de escritura
3. **`helpers`** (`utils/`) — utilidades internas (deepClone, deepEqual, generateId, etc.)

Estas tres pezas son **autónomas**: non dependen de TreeEngine, StateStore, nin de ningunha outra clase que aínda non existe. Por iso son o punto de partida ideal.

**NON** crear: TreeEngine, StateStore, UnlockResolver, nin ningunha outra clase (vén despois).
...
- **Tipado sobre `EventMap`** (definido en `types/events.ts`). Os métodos `on`, `off`, `emit` son type-safe.
- **`on()` devolve unha función de unsubscribe** (patrón moderno, máis ergonómico que `off()` manual).
- **`off()` tamén existe** para compatibilidade e casos onde se garda a referencia.
- **Emit é síncrono** — os handlers execútanse na orde de rexistro.
- **Erros nun handler non rompen o emit** — captúranse e (opcionalmente) reportanse, pero os demais handlers execútanse igual.
- **`once()`** para handlers que se auto-eliminan tras a primeira execución.
- **`removeAllListeners(event?)`** para limpeza.

### 4.2 ConcurrencyGuard

- **Serializa operacións async** — se chega unha operación mentres outra está en curso, encólase.
- **Patrón:** cola de promesas. Cada `run(fn)` espera a que termine a anterior.
- **`runExclusive(fn)`** é o método principal.
- **Detección de operación en curso:** `isLocked()` getter.
- **NON é un mutex re-entrante** — se unha operación dentro doutra chama a `runExclusive`, NON debe deadlockear. Para iso, a operación interna debe usar a versión "unsafe" ou o guard debe detectar reentrada. **Decisión: simple, non re-entrante. As operacións do motor non se aniñan.**
- **Timeout opcional** — se unha operación tarda máis de X ms, rexéitase con erro `OPERATION_LOCKED`.

### 4.3 Helpers

Utilidades puras en `utils/`:
- **`deepClone`** — clon profundo (structuredClone se está dispoñible, fallback manual)
- **`deepEqual`** — comparación profunda de valores
- **`generateId`** — xerador de IDs únicos (prefixo + timestamp + random)
- **`isPlainObject`** — type guard para obxectos planos
- **`clamp`** — limita un número entre min/max
...
- Cero `any`
- Cobertura **100%** nestas tres pezas (son lóxica pura, testables completamente)
...
```bash
git status                        # Clean
git log --oneline -3              # Último commit: o de 1.4
pnpm check-env                    # Pasa
pnpm validate                     # Pasa
```
...
#### 5.1.1 packages/core/src/utils/deepClone.ts
...
#### 5.1.2 packages/core/src/utils/deepEqual.ts
...
#### 5.1.3 packages/core/src/utils/generateId.ts
...
#### 5.1.4 packages/core/src/utils/guards.ts
...
#### 5.1.5 packages/core/src/utils/index.ts
...
### 5.2 Crear engine/EventEmitter.ts
...
### 5.3 Crear engine/ConcurrencyGuard.ts
...
### 5.5 Actualizar packages/core/src/index.ts
...
⚠️ **NON exportamos `utils/` na API pública.** Os helpers son de uso interno. Mantéñense importables internamente vía `../utils/index.js` pero non se reexportan dende o index raíz.

### 5.6 Tests

#### 5.6.1 packages/core/__tests__/utils/deepClone.test.ts
...
#### 5.6.2 packages/core/__tests__/utils/deepEqual.test.ts
...
#### 5.6.3 packages/core/__tests__/utils/generateId.test.ts
...
#### 5.6.4 packages/core/__tests__/utils/guards.test.ts
...
#### 5.6.5 packages/core/__tests__/engine/EventEmitter.test.ts
...
#### 5.6.6 packages/core/__tests__/engine/ConcurrencyGuard.test.ts
...
⚠️ **Cobertura esperada en core:** 100% nas tres pezas novas (EventEmitter, ConcurrencyGuard, helpers). Son lóxica pura e completamente testable. Se non chega a 100%, **detente e reporta** que falta cubrir.

⚠️ **Tests async:** os tests de ConcurrencyGuard usan timers e delays. Se algún test é flaky (pasa ás veces, falla outras), reporta — pode requirir axuste de tempos.

### 5.8 Actualizar packages/core/README.md

Engadir despois da sección "Errors":

```markdown
### Engine primitives

\`\`\`typescript
import { EventEmitter, ConcurrencyGuard } from '@yggdrasil-forge/core'

// Typed event emitter (over EventMap)
const emitter = new EventEmitter()
const unsubscribe = emitter.on('unlock', (nodeId, instance) => {
  console.info(\`\${nodeId} unlocked at tier \${instance.currentTier}\`)
})

// Serializes async operations to prevent race conditions
const guard = new ConcurrencyGuard({ timeoutMs: 5000 })
await guard.runExclusive(async () => {
  // ...exclusive operation
})
\`\`\`
...
- **Minor** (non major)
- Summary: `Add first engine primitives: typed EventEmitter (over EventMap), ConcurrencyGuard (serializes async operations with optional timeout), and internal utilities (deepClone, deepEqual, generateId, isPlainObject, clamp).`
...
```markdown
### Added
- `@yggdrasil-forge/core`: first engine primitives
  - `EventEmitter` — typed event emitter over `EventMap` (on/once/off/emit, returns unsubscribe, error isolation, listenerCount, removeAllListeners)
  - `ConcurrencyGuard` — serializes async operations (runExclusive, isLocked, pendingCount, drain, optional timeout)
  - Internal utilities: `deepClone`, `deepEqual`, `generateId`, `isPlainObject`, `clamp`
- 100% test coverage for engine primitives
```
...
git add .
git status
git commit -m "feat(core): add EventEmitter, ConcurrencyGuard, and internal utilities"
...
- **Evitar `sed` para edicións multi-liña.**

---

## 7. QUE NON FACER

- ❌ Non crear TreeEngine, StateStore, UnlockResolver, nin ningunha outra clase (sub-fases posteriores).
- ❌ Non exportar `utils/` na API pública (son helpers internos).
- ❌ Non facer o ConcurrencyGuard re-entrante (decisión: simple, non re-entrante).
...
1. ✅ `packages/core/src/utils/` con deepClone, deepEqual, generateId, guards, index
2. ✅ `packages/core/src/engine/EventEmitter.ts`
3. ✅ `packages/core/src/engine/ConcurrencyGuard.ts`
4. ✅ `packages/core/src/engine/index.ts`
5. ✅ `packages/core/src/index.ts` actualizado (exporta engine, NON utils)
6. ✅ 6 ficheiros de tests (4 utils + 2 engine)
7. ✅ Cobertura 100% nas pezas novas
8. ✅ `packages/core/README.md` actualizado
9. ✅ Changeset minor creado
10. ✅ CHANGELOG raíz actualizado
11. ✅ Build, test, validate pasan
12. ✅ CI verde

Mostra ao autor:
```bash
ls packages/core/src/engine/
ls packages/core/src/utils/
...
```
═══════════════════════════════════════
SUB-FASE 1.5 — COMPLETADA
═══════════════════════════════════════

✅ Primeiras pezas do motor:
   - EventEmitter (tipado sobre EventMap)
   - ConcurrencyGuard (serializa async, timeout opcional)
   - utils: deepClone, deepEqual, generateId, isPlainObject, clamp

✅ Tests: [N] tests pasan ([M] novos)
✅ Cobertura: 100% nas pezas novas
...
[Lista, ou "Ningún"]
[Se houbo tests flaky, indícao]

📊 Decisións do executor:
[Lista, ou "Ningunha"]

📋 Estado:
LISTO PARA PROCEDER Á SUB-FASE 1.6
(StateStore con Immer + cache invalidation)
```

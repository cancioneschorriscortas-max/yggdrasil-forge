...
Want to see **composition** instead of polish? The
[`curriculum`](./examples/curriculum) example builds a nested course: modules are
sub-trees you drill into, and a module only unlocks once its prerequisite module
is 100% complete (cross-tree gating via `subtree_completion`). Minimal styling, on
purpose — it's about structure, not looks.

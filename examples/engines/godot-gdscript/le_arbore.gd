...
#
# Uso: copia un ficheiro da galería (p.ex. panadeiro.json) como
...
func _init() -> void:
	...
	for recurso: Dictionary in arbore.get("resources", []):
		print("Recurso %s — inicial: %s" % [_label(recurso["label"]), recurso.get("initial", 0)])
	...
	for nodo: Dictionary in arbore["nodes"]:
		...
		if nodo.has("costPerTier"):
			...
			for c: Dictionary in primeiro:
				partes.append("%s×%s" % [c["amount"], c["resourceId"]])
			custo = ", ".join(partes)
